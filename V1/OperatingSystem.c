...
// Array that contains the names of the processes states
...
int OperatingSystem_LongTermScheduler() {
	...
	for (i=0; programList[i]!=NULL && i<PROGRAMSMAXNUMBER ; i++) {
		...
		switch (PID)
		{
		// A process should not be able to exist if its program does not exist
		case PROGRAMDOESNOTEXIST:
			ComputerSystem_DebugMessage(104, ERROR, programList[i]->executableName, "--- it does not exist ---");
			break;
		
		// A process should not be able to exist if its program is not valid
		case PROGRAMNOTVALID:
			ComputerSystem_DebugMessage(104, ERROR, programList[i]->executableName, "--- invalid priority or size ---");
			break;
		
		// A process should not be able to exist if there is not a valid process table entry is not available
		case NOFREEENTRY:
			ComputerSystem_DebugMessage(103, ERROR, programList[i]->executableName);
			break;
		
		// A process that is too big should not be able to exist
		case TOOBIGPROCESS:
			ComputerSystem_DebugMessage(105, ERROR, programList[i]->executableName);
			break;
		
		// A valid program should be able to exist
		default:
...
int OperatingSystem_CreateProcess(int indexOfExecutableProgram) {
	...
	// Check if the operating system was able to obtain an entry in the process table
	if (PID == NOFREEENTRY)
		return NOFREEENTRY;
	...
	loadingProgram=OperatingSystem_LoadProgram(programFile, loadingPhysicalAddress, processSize);
	if (loadingProgram == TOOBIGPROCESS)
		return TOOBIGPROCESS;
...
int OperatingSystem_ExtractFromReadyToRun() {
	...
	// Extract first process of the queue with most priority, if it has no processes, continue for all the queues ordered by priority
	for (int queue = 0; queue < NUMBEROFQUEUES; queue++){
		...
		if (selectedProcess != NOPROCESS){
			// Return most priority process or NOPROCESS if all queues are empty
			return selectedProcess;
		}
	}
	return NOPROCESS;
...
void OperatingSystem_HandleSystemCall() {
	...
	switch (systemCallID) {
		...
		case SYSCALL_YIELD:
			// Show message: "Process [executingProcessID] will transfer the control of the processor to process [newExecutingProcessID]\n"
			queueID=processTable[executingProcessID].queueID;
			selectedProcessID = Heap_getFirst(readyToRunQueue[queueID], numberOfReadyToRunProcesses[queueID]);

			if (selectedProcessID == NOPROCESS){
				break;
			}
			if (processTable[selectedProcessID].priority == processTable[executingProcessID].priority){
				ComputerSystem_DebugMessage(115,SHORTTERMSCHEDULE,executingProcessID,programList[processTable[executingProcessID].programListIndex]->executableName,selectedProcessID,programList[processTable[selectedProcessID].programListIndex]->executableName);
...
// Prints the Ready-To-Run Queue
void OperatingSystem_PrintReadyToRunQueue(){
	int PID;
	ComputerSystem_DebugMessage(106, SHORTTERMSCHEDULE);
	// USER
	ComputerSystem_DebugMessage(112, SHORTTERMSCHEDULE);
	for (int i = 0; i < numberOfReadyToRunProcesses[USERPROGRAM]; i++){
		PID = readyToRunQueue[USERPROGRAM][i].info;
		ComputerSystem_DebugMessage(114, SHORTTERMSCHEDULE, PID, processTable[PID].priority);
		if (i != numberOfReadyToRunProcesses[USERPROGRAM] - 1){
	...
	for (int j = 0; j < numberOfReadyToRunProcesses[DAEMONPROGRAM]; j++){
		PID = readyToRunQueue[DAEMONPROGRAM][j].info;
		ComputerSystem_DebugMessage(114, SHORTTERMSCHEDULE, PID, processTable[PID].priority);
		if (j != numberOfReadyToRunProcesses[DAEMONPROGRAM] - 1){
			ComputerSystem_DebugMessage(108, SHORTTERMSCHEDULE);
		}
	}
	ComputerSystem_DebugMessage(109, SHORTTERMSCHEDULE);
